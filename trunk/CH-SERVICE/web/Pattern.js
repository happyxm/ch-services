function Pattern() {
	...
	/*
	 * Weights for judging a captioned node match These must sum to 1.
	 */
	const
	KEYWORD_FACTOR = 0.7;
	const
	...
	/**
	 * Because textual matches to the right are less likely than textual matches
	 * to the left for a textbox, the strength of a "right" match is reduced by
	 * a constant factor
	 */
	const
	TEXTBOX_FACTORS = {
	...
	var typeSuffixRegexp = new RegExp('\\b(' + typeNames.join('|') + ')\\s*$',
			'i');
	 ...
	 * Find a pattern.
	 * 
	 * doc: document to search. (If context argument is provided, then this
	 * argument is ignored and context.document is used instead.)
	 * 
	 * pattern: pattern to search for, which may be:
	 * 
	 * a keyword pattern, represented as a string a TC pattern, represented as a
	 * Javascript object of type TC an XPath pattern, represented as a
	 * Javascript object of type XPath
	 * 
	 * If pattern is a keyword pattern and ends with a type name (e.g.
	 * "textbox"), then strip that type name from the keyword pattern and use it
	 * instead of the types argument.
	 * 
	 * Pattern may be null, which indicates that the caller just wants the nodes
	 * of the given types. (Used, for example, by enter() with only one
	 * argument.)
	 * 
	 * types: types of nodes to search. Array may contain one or more of
	 * Pattern.LINK, Pattern.BUTTON, etc. May be null, which indicates that the
	 * caller doesn't care.
	 * 
	 * context: optional range in which matches must appear
	 * 
	 * requires: (1) either types or pattern is non-null; (2) if pattern is a
	 * keyword pattern, then either types is non-null or pattern ends with a
	 * type name
	 * 
	 * returns: Match iteration: If pattern was a TC pattern, the Match
	 * iteration contains all the regions it matches.
	 * 
	 * If pattern was a keyword pattern, the Match iteration contains the
	 * highest-strength match (plus any matches that are tied with it in
	 * strength)
	 */
	...
	/* optional Range */context) {
		if (context)
			doc = context.startContainer.ownerDocument;
		...
		if (instanceOf(pattern, Match))
			return oneMatch(pattern);
		...
		if (instanceOf(pattern, XPath)) {
			var contextNode = context ? rangeToContainer(context)
					: doc.documentElement;
			var result = doc.evaluate(pattern.xpathExpression, contextNode,
					pattern.namespaceResolver, pattern.resultType, null); // create
																			// new
																			// result
			...
			case XPathResult.NUMBER_TYPE:
				throw new Error("The pattern \"" + pattern.xpathExpression
						+ "\" did not match part of the web page.");
			...
			default:
				Test.fail("Unrecognized XPathResult type: "
						+ pattern.resultType);
			}
		}
		...
		// TODO: XUL should throw error here
		if (instanceOf(pattern, TC)) {
			return pattern.find(doc, context);
		}
	...
	function findKeywordsInAllFrames(/* Document */doc, /* String[] */types, /* Keywords */
			keywords, /* optional Range */context) {
		...
		if (matches.length == 0) {
			for ( var j = 0; j < docs.length; ++j) {
				var frameDoc = docs[j];
				matches = findFieldsByInternalName(frameDoc, types, keywords,
						context);
	...
	/**
	 * Find all fields of the given types whose name attribute matches the
	 * pattern exactly. Return InternalMatches[] array.
	 */
	function findFieldsByInternalName(/* Document */doc, /* String[] */types, /* Keywords */
			keywords, /* optional Range */context) {
		var matches = [];
		for ( var i = 0; i < types.length; i++) {
			var type = types[i].toLowerCase();
	...
	/*
	 * Find a keyword pattern. Returns a Match iteration.
	 */
	function findKeywords(/* Document */doc, /* String[] */types, /* Keywords */
			keywords, /* optional Range */context) {
		var matches = findKeywordsInAllFrames(doc, types, keywords, context);
		// if no matches to pattern, then maybe it contains an ordinal, e.g.,
		// "second textbox"
		var ordinalInfo;
		if (matches.length != 1) {
			ordinalInfo = extractOrdinalInfo(keywords);
			if (ordinalInfo) {
				matches = findKeywordsInAllFrames(doc, types,
						ordinalInfo.keywords, context);
		...
		if (ordinalInfo) {
			if (ordinalInfo.ordinal > bestMatches.length) {
				// can't select the 5th textbox if there are only 4 textbox
				// matches
				return EMPTY_MATCH;
			} else {
		...
		/*
		 * for (var i = bestMatches.length - 1; i >= 0; --i) { delete
		 * bestMatches[i].node.wrappedJSObject[marker]; }
		 */
	...
	// regexp matches a string that starts with an ordinal such that the ordinal
	// is terminated
	// by either a space or a word boundary, so it will match "second place" but
	// not "secondhand store"
	var ordinalRe = new RegExp("^((\\d+)th|" + ordinalStr.join("|") + ")$", "i");

	/**
	 * Given a keyword pattern, such as "1st button" or "third textbox", find
	 * the value of the ordinal if one is found at the beginning of the pattern.
	 * Ordinal values are 1-based, so for example, extractOrdinalInfo("third
	 * textbox") would return 3 as the ordinal.
	 * 
	 * If an ordinal is found, then an object is returned with two properties
	 * defined: ordinal: the ordinal value as a positive integer keywords:
	 * Keywords object without the text that the ordinal contributed
	 * 
	...
	function extractOrdinalInfo(/* Keywords */keywords) {
		...
		if (m[2] && m[2].length) { // matches "Nth"
			...
			Test.fail(keywords
					+ " matched ordinalRe but not extractOrdinalInfo()");
		}
	...
	/* optional Range */context) {
		...
		if (element)
			matches.push(new InternalMatch(element, 1.0));
	}
	...
	/**
	 * Finds each clickable-input match to pattern and adds it to the matches
	 * array.
	 */
	...
	/* optional Range */context) {
		...
		// findSelfLabeledNodes() is not appropriate here because it is
		// difficult
		// to define an appropriate nodeFilter. We want to consider all
		// textblobs
		// under the root node, but findSelfLabeledNodes() only matches one
		// textblob
		// per node that matches the filter, whereas there may be multiple
		// literal matches
		// under the root node.
		var root = getFindRoot(doc, context);
		...
		var blob;
		// TODO: for simple text, may occur more than once in a blob; need to
		// match all instances
	...
	/**
	 * Finds each link match to pattern and adds it to the matches array.
	 */
	...
	/**
	 * Finds each textbox match to pattern and adds it to the matches array.
	 */
	function findTextboxMatches(/* InternalMatch[] */matches,
	/* Document */doc,
	/* Keywords */keywords,
	/* optional Range */context) {
		findCaptionedNodes(matches, doc, keywords, isTextbox, TEXTBOX_FACTORS,
				context);
	}

	/**
	 * Finds each checkbox match to pattern and adds it to the matches array.
	 */
	...
	/* optional Range */context) {
		if (instanceOf(doc, XULDocument)) {
			findSelfLabeledNodes(matches, doc, keywords, isCheckbox, context);
		} else {
			findCaptionedNodes(matches, doc, keywords, isCheckbox,
					CHECKBOX_FACTORS, context);
		}
	}

	/**
	 * Finds each radiobutton match to pattern and adds it to the matches array.
	 */
	...
	/* optional Range */context) {
		if (instanceOf(doc, XULDocument)) {
			findSelfLabeledNodes(matches, doc, keywords, isRadioButton, context);
		} else {
			findCaptionedNodes(matches, doc, keywords, isRadioButton,
					CHECKBOX_FACTORS, context);
		}
	}

	/**
	 * Finds each listbox match to pattern and adds it to the matches array.
	 */
	...
	/* optional Range */context) {
		if (instanceOf(doc, XULDocument)) {
			selfLabeled = findSelfLabeledNodes(matches, doc, keywords,
					isListbox, context);
			if (selfLabeled == null) {
				return findCaptionedNodes(matches, doc, keywords, isListbox,
						LISTBOX_FACTORS, context);
			} else {
				return selfLabeled
			}
		} else {
	...
	/**
	 * Finds each listitem match to pattern and adds it to the matches array.
	 */
	...
	/* optional Range */context) {
		...
		if (!keywords || !keywords.length) {
			if (Pattern.debugPatternMatching)
				debug("returning all nodes of specified type");
			...
			if (Pattern.debugPatternMatching)
				debug("found " + matches.length + " such nodes");
		} else {
			var node = nodeIterator.nextNode();
			while (node) {
	...
	/**
	 * Find input elements (textboxes, checkboxes, radiobuttons) by looking for
	 * keywords in nearby labels.
	 * 
	 */
	...
	/* optional Range */context) {
		...
		/*
		 * Now handled by findFieldsByInternalName // look for form element with
		 * exact name nodes = info.getFormElements(pattern); for (var i = 0; i <
		 * nodes.length; i++) { if (filter(nodes[i])) { matches.push(new
		 * InternalMatch(nodes[i], 3)); } }
		 */
		...
		while (blob = blobIterator.next()) {
			if (Pattern.debugPatternMatching)
				debug("testing text blob: " + blob);
			var metric = keywords.match(blob.value);
			if (metric <= STRENGTH_THRESHOLD)
				continue;
			if (Pattern.debugPatternMatching)
				debug("matched with strength " + metric);
			...
			if (label != null) {
				...
				if (node != null && filter(node)) {
					if (Pattern.debugPatternMatching)
						debug("best match is labeled node: " + node);
					matches
							.push(new InternalMatch(node, LABEL_FACTOR * metric));
					continue;
				}
			}
			...
			for ( var j = 0; j < nodes.length; j++) {
				...
				// Next, see if blob is nested inside the node itself (e.g.,
				// SELECT elements).
				if (isDescendentOf(blob.firstNode, node)) {
					if (Pattern.debugPatternMatching)
						debug("found blob " + blob + " nested inside " + node);
				...
				if (Pattern.debugPatternMatching)
					debug("boxNode=" + boxNode);

				// search for a relationship between boxLabel and boxNode, with
				// increasing
				...
				if (!relation)
					relation = boxLabel.relatedTo(boxNode, tolerance = 0);
				if (!relation)
					relation = boxLabel.relatedTo(boxNode, tolerance = 5);
				...
				case "left":
					...
					overlap = computeOverlap(boxNode.y1, boxNode.y2,
							boxLabel.y1, boxLabel.y2);
					break;
				case "right":
				...
				case "above":
					...
					overlap = computeOverlap(boxNode.x1, boxNode.x2,
							boxLabel.x1, boxLabel.x2);
					break;
				case "below":
					distance = computeDistance(boxNode.y1, boxLabel.y2);
					overlap = computeOverlap(boxNode.x1, boxNode.x2,
				...
				if (Pattern.debugPatternMatching)
					debug(relation + " distance=" + distance + " overlap="
			...
			if (bestNode != null) {
				if (Pattern.debugPatternMatching)
					debug("best match is " + bestStrength + " "
							+ flattenDom(bestNode)[0]);
				matches.push(new InternalMatch(bestNode, bestStrength));
			}
		}

		// computes strength as an inverse function of distance between two
		// pixel coordinates;
		...
		// e.g. computeOverlap(0,4, -2, 1) = 0.333 since [0,4] and [-2,1]
		// intersect in [0,1] whose
	...
	function makeFilter(/* Node->boolean */nodeTypeFilter, /* optional Range */
			context) {
	...
	/* optional Range */context) {
		...
		if (root.tagName != 'TABLE')
			throw new Error("cannot look for rows if not a table");
		var rows = root.rows;
	...
	/* optional Range */context) {
		var root = getFindRoot(doc, context);
		if (root.tagName != 'TR')
			throw new Error("cannot look for cells if not a row");
	...
	/**
	 * Given a match for a cell, returns as successive matches intersecting
	 * cells below the cell originally matched.
	 */
	...
	/* optional Range */context) {
		var root = getFindRoot(doc, context);
		if (root.tagName != 'TD')
			throw new Error("need a cell to get its column")
	...
	/* Keywords */keywords,
	/* optional Range */context) {
		findCaptionedNodes(matches, doc, keywords, isSignificantImage,
				IMAGE_FACTORS, context);
	}
	...
	function getFindRoot(/* Document */doc, /* optional Range */context) {
		if (context)
			return rangeToContainer(context);
		else if (doc.body)
			return doc.body;
		else
...
InternalMatch.prototype.toString = function() {
	return '[' + this.strength.toFixed(2) + ': ' + this.node + ' ]';
}